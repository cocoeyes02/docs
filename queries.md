...
> {note} `whereRaw` with OR clause cannot use [`SoftDeletes`](/docs/{{version}}/eloquent#soft-deleting) trait safely. If you use OR clause in `whereRaw`, you should use SQL Parentheses in `whereRaw`, closure to `where` or `orWhere` instead of `whereRaw`.
